# region Практическая работа.
# Цели практической работы
# Применить рекурсивный метод для решения задач.
# Научиться находить и исправлять проблемы, которые могут возникнуть при передаче изменяемых типов данных в качестве
# параметров функции.
# Научиться отличать именованные аргументы и значения по умолчанию, а также использовать их при написании функций, чтобы
# избежать повторов и создавать более гибкий код.
...
#
#
# region Задача 1. Challenge-2
# Что нужно сделать
# Вдохновившись мотивацией Антона, ваш друг тоже решил поставить перед собой задачу, но не напрямую связанную с
# математикой, а именно: написать функцию, которая выводит все числа от 1 до num без использования циклов. Помогите
# другу реализовать такую функцию.
...
def function(num_1, end):
    if num_1 > end:
        return
    else:
        print(num_1)
        return function(num_1 + 1, end)


num = function(1, int(input("Введите num: ")))
...
# endregion Задача 1. Challenge-2
# region Задача 2. Поиск элемента — 2
# Что нужно сделать
...
# Основной функционал описан в отдельной функции(-ях).
# Переменные и функции имеют значимые имена, не только a, b, c, d.
# endregion Задача 2. Поиск элемента — 2
# region Задача 3. Глубокое копирование (нерешена)
# Что нужно сделать
...
#
# sites = []
# number_sites = input("Сколько сайтов: " )
# name_product = str(input("Введите название продукта для нового сайта: "))
#
# Сайт для iPhone:
# site = {
# 'html': {
...
# endregion Задача 3. Глубокое копирование
# region Задача 4. Продвинутая функция sum (нерешена)
# Что нужно сделать
...
# Переменные и функции имеют значимые имена, не только a, b, c, d.
#
# endregion Задача 4. Продвинутая функция sum (нерешена)
# region Задача 5. Список списков — 2 (нерешена)
# Что нужно сделать
# Вы уже работали с многомерными списками и решали задачи, где с помощью list comprehensions «выпрямляли» многомерные
# списки в один. Это не получится, если списков неограниченное количество и у элементов разные уровни вложенности.
#
# Дан такой список:
#
...
#
# Функция должна получать список и возвращать его раскрытую версию (не нужно добавлять элементы в список, записанный в
# глобальную переменную, созданную снаружи функции).
#
# Подсказка
#
...
# Переменные и функции имеют значимые имена, не только a, b, c, d.
#
# endregion Задача 5. Список списков — 2 (нерешена)
# region Задача 6. Быстрая сортировка (нерешена)
# Что нужно сделать
# Реализуйте алгоритм быстрой сортировки (её называют сортировкой Хоара).
#
# За один шаг алгоритма выполните следующие действия:
#
# Выберите один элемент списка (его иногда называют опорным элементом). Сделать это можно разными способами, но важно
# придерживаться одного принципа. В нашем случае опорным элементом всегда будет крайний правый (например, в списке
# [1, 2, 3] это 3).
...
# [8, 8];
# [9, 9].
# Для списка с элементами меньше опорного ([5, 4, 2, 1]) и списка с элементами больше опорного ([9, 9]) выполните те же
# шаги заново — запустите рекурсию.
...
#
# С [9, 9] всё просто. Элементов меньше или больше опорного нет, поэтому рекурсия не пойдёт глубже, а вызов рекурсии по
# списку [9, 9] быстро завершится и вернёт этот же список (его и не нужно было сортировать).
#
...
# Описания коммитов осмыслены и понятны: 111, done, «я сделалъ» — неверно, added m15 homework, 14.3 fix: variables
# naming — верно.
...
# endregion Задача 6. Быстрая сортировка (нерешена)
# endregion Практическая работа.
