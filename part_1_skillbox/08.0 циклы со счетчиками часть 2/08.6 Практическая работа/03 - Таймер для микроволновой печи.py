...
# В нашем случае будем выводить в консоль сообщение с обратным отсчетом в секундах от “reverse_timer” до
# момента готовности, то есть «0» секунд, и спрашивать пользователя, готов ли он забрать еду.
# Пользователь в любой момент может прервать режим разогрева, введя «1» (то есть ответить «Да, еда готова»),
# огда программа выводит на экран сообщение «Ваша еда готова, можете забрать» и показывает, на какой секунде был прерван таймер.
...
reverse_timer = int(input("Сколько секунд разогревать еду: "))
time = 0
for sec in range(reverse_timer, 0, -1):
    stop = int(input(f"Прервать режим разогрева на {sec } секунде или нет (1 - да, 0 - нет): "))
    if sec == 1 and stop == 0:
        print("Ваша еда готова, осторожно горячо! ")
        time += 1
    elif sec > 1 and stop == 1:
        print(f"Ваша еда готова на {sec} секунде. Можете забрать.")
        break
